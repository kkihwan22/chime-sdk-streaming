...
@Component
public class TuteeRoleStrategy implements RoleStrategy {
    ...
    public Attendee updateRole(Live live, SpaceUser spaceUser) {
        ...
        attendee.changedRoleForTutee(live);
        attendeeDataprovider.save(attendee);
        spaceComponent.publish(new SpaceBody(
                "attendee_action",
                live.getChannelId(),
                new ActionMessage("screen_share_lock", Collections.singletonList(new AttendeeResponse(attendee)))));
